...
export class TableGroupsPipe implements PipeTransform {
  ...
  transform(bills: ExtendedBillConstructor[]): {table:{id:string,name:string},bills:ExtendedBillConstructor[]}[] {
    let groups = bills.reduce((acc,bill) => {
      ...
      return acc;
    },[] as {table:{id:string,name:string},bills:ExtendedBillConstructor[]}[])
    return groups;
  }
