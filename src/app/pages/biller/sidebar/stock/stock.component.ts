...
export class StockComponent implements AfterViewInit {
  ...
  constructor(
    ...
    this.closeStockValuePanelSubscription
      .pipe(debounceTime(600))
      .subscribe((data) => {
        this.isStockValuationOpen = data;
      });
    this.cashCounterPanelSubscription
      .pipe(debounceTime(600))
      .subscribe((data) => {
  ...
  ngAfterViewInit(): void {
    if (this.dataProvider.touchMode) {
      ...
      var mc = new Hammer.Manager(
        document.getElementById('stockValuationTrigger'),
      );
      ...
      // cashCounterTrigger
      var mc = new Hammer.Manager(
        document.getElementById('cashCounterTrigger'),
      );
      mc.add(new Hammer.Press({ time: 500 }));
      mc.on('press', (ev: any) => {
        //  console.log("press",ev);
