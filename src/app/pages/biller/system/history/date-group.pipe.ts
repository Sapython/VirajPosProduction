...
export class DateGroupPipe implements PipeTransform {
  ...
  transform(bills: ExtendedBillConstructor[]): {date:Date,bills:ExtendedBillConstructor[]}[] {
    let groups = bills.reduce((acc,bill) => {
      let index = acc.findIndex((group) => {
        ...
        return billDate.getDate() == groupDate.getDate() && billDate.getMonth() == groupDate.getMonth() && billDate.getFullYear() == groupDate.getFullYear();
      });
      if(index == -1){
