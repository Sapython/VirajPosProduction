...
export async function validateAny(
  ...
  if (typeof value !== type) {
    throw new HttpsError('invalid-argument', 'Invalid value');
  }
...
export function validateName(name: string) {
  if (typeof name != 'string' || !name || name.length < 3 || name.length > 20) {
    throw new HttpsError(
      'invalid-argument',
...
export function validatePhone(phone: string) {
  if (typeof phone != 'string' || !phone || phone.length != 10) {
...
export function validateBusiness(business: any) {
  if (
    ...
    throw new HttpsError('invalid-argument', 'Invalid business object');
  }
  return true;
}
